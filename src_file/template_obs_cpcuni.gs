'reinit'
'open cpcuni_rev.ctl'
'open cpcuni_clim_rev.ctl'
'open src_file/Africa_vhimasks0p036.ctl'
...
'set time date1'
*
'define pr = tpr*ave(rain.1/10, time=date1,time=date2)'
'define pcl = tpr*ave(rain.2/10, time=date1,time=date2)'

'define pobs = lterp(pr,mask.3(t=1))'
'define pclim = lterp(pcl,mask.3(t=1))'

'define pobs=if(pobs,<,0,0,pobs)'
...
if (masksplot = 1)
 'define pobs=maskout(pobs,mask.3(t=1))'
 'define pclim=maskout(pclim,mask.3(t=1))'
endif
'define drymask = maskout(drymask,mask.3(t=1))'
...
'set string 1 c 6'
'set strsiz  0.2'
'draw string 'xmid' 'ytop+0.625' CPC-Uni xday-Day Total Precip ('varunit')'
...
***************************** Plot the Total Observed Rainfall *****************************
'set string 1 c 1 0'
*
'set mpdraw off'
*
...
'set strsiz  0.2'
'draw string 'xmid' 'ytop+0.625' CPC-Uni xday-Day Total Precip ('varunit')'
'draw string 'xmid' 'ytop+0.25'  Obs. Period: date1 to date2'
...
'set line 1 1 6'
'draw shp src_file/AFR_adm1'
'printim Africa_rev_rfe_obs_precip.png '
'clear'
